...
export default function ProductCard({ product, index }: ProductCardProps) {
  ...
  const handleAddToCart = (e: React.MouseEvent) => {
    ...
    trackEvent('AddToCart', {
      ...
      value: isBundle ? product.price : calculatePrice(selectedGrams),
      currency: 'CZK',
      ...
      <div className="relative rounded-3xl overflow-hidden">
        ...
        <div className="relative p-8">
          ...
          <div className="space-y-4">
            ...
            {!isBundle && (
              <div className="flex flex-wrap items-center justify-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all duration-300">
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-white bg-gradient-to-r from-purple-500 to-purple-600 shadow-lg whitespace-nowrap">
                  THC-X {product.thc_x_percent || '40.00'}%
                </span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-white bg-gradient-to-r from-emerald-500 to-emerald-600 shadow-lg whitespace-nowrap">
                  CBD {product.cbd_percent || product.cbd_percentage || '9.05'}%
                </span>
                <span className="px-2 py-0.5 rounded-full text-[10px] font-bold text-white bg-gradient-to-r from-amber-500 to-amber-600 shadow-lg whitespace-nowrap">
                  CBG {product.cbg_percent || product.cbg_percentage || '0.10'}%
                </span>
              </div>
            )}
